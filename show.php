...
$is_admin = isset($_SESSION['user_role']) && $_SESSION['user_role'] === 'admin'; // Check if user is admin
?>
      ...
      <table>
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<td>" . $row["Timestamp"] . "</td>"; // Timestamp column data
                
                if ($is_logged_in && $is_admin) {
                    echo "<td>";
                    echo "<a href='edit.php?id=" . $row["id"] . "'>Edit</a> | ";
                    echo "<a href='delete.php?id=" . $row["id"] . "' onclick='return confirm(\"Are you sure?\")'>Delete</a> | ";
                    echo "<a href='resend.php?id=" . $row["id"] . "'>Resend Code</a>";
                    echo "</td>";
                }
                echo "</tr>";
            ...
            echo "<tr><td colspan='" . ($is_logged_in && $is_admin ? "9" : "8") . "' class='no-records'>No records found.</td></tr>";
        }
        ?>
        </tbody>
        <?php if ($is_logged_in && $is_admin): ?>
